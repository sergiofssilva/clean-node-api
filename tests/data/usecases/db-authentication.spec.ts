...
import { mockAccountModel, throwError, mockAuthenticationParams, mockAuthenticationModel } from '@/tests/domain/mocks'
...
describe('DbAuthentication Usecase', () => {
  ...
  test('Should call HashComparer with correct values', async () => {
    ...
    await sut.auth(mockAuthenticationParams())
    expect(compareSpy).toHaveBeenCalledWith(mockAuthenticationParams().password, mockAccountModel().password)
  })
  ...
  test('Should return an AuthenticationModel if Encrypter succeed', async () => {
    const { sut } = makeSut()
    const authenticationModel = await sut.auth(mockAuthenticationParams())
    expect(authenticationModel).toEqual(mockAuthenticationModel())
  })
